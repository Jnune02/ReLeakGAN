...
def vector_data_loader(positive_filepath, batch_size, shuffle, num_workers, pin_memory):
    dataset = Vector_Dataset(positive_filepath)
...
def dis_l2_data_loader(positive_data, negative_data, batch_size, shuffle, num_workers, pin_memory):
    # Instead of re-implementing an l2 and l3 version of Dis_Dataset,
    # try to use the method first.
    dataset = Dis_Dataset(positive_data, negative_data)
    return DataLoader(dataset=dataset, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers, pin_memory=pin_memory)
